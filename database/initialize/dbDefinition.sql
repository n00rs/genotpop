CREATE TABLE IF NOT EXISTS tbl_user (
  ...
  dat_modified TIMESTAMP WITH TIME ZONE DEFAULT now(),
...
CREATE INDEX idx_user_email ON tbl_user(vchr_email);
...
CREATE INDEX idx_category_code_and_name ON tbl_category(vchr_category_code, vchr_category_name);
...
CREATE INDEX idx_stock_code_and_name ON tbl_stock(vchr_stock_code, vchr_stock_name);
...
CREATE TABLE IF NOT EXISTS tbl_customer (
  ...
  vchr_address TEXT,
  dbl_discount DECIMAL(10,2) DEFAULT 0.00,
...
CREATE INDEX idx_customer_code_and_name ON tbl_customer(vchr_customer_code, vchr_customer_name);
...
CREATE TABLE IF NOT EXISTS tbl_sales_master (
  ...
  dat_document_date DATE NOT NULL,
  ...
  dble_before_vat DECIMAL(10,2) NOT NULL CHECK (dble_before_vat >= 0),
...
CREATE TABLE IF NOT EXISTS tbl_sales_master_details (
  ...
  dbl_discount DECIMAL(10,2) DEFAULT 0.00 CHECK (dbl_discount >= 0),
  dbl_unit_price DECIMAL(10,2) NOT NULL CHECK (dbl_unit_price >= 0),
  vchr_refference VARCHAR (200),
